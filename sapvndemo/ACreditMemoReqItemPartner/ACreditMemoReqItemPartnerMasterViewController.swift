//
// ACreditMemoReqItemPartnerMasterViewController.swift
// sapvndemo
//
...
class ACreditMemoReqItemPartnerMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [ACreditMemoReqItemPartnerType] = [ACreditMemoReqItemPartnerType]()
    private let logger = Logger.shared(named: "ACreditMemoReqItemPartnerTypeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let acreditmemoreqitempartnertype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CreditMemoRequest", value: "\(acreditmemoreqitempartnertype.creditMemoRequest!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.apicreditmemorequestsrvEntities.fetchACreditMemoReqItemPartner(matching: query) { aCreditMemoReqItemPartner, error in
            guard let aCreditMemoReqItemPartner = aCreditMemoReqItemPartner else {
                completionHandler(error!)
                return
            }
            self.entities = aCreditMemoReqItemPartner
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ACreditMemoReqItemPartnerDetailViewController
...
extension ACreditMemoReqItemPartnerMasterViewController: EntitySetUpdaterDelegate {
