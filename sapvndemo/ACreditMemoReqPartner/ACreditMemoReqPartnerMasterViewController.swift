//
// ACreditMemoReqPartnerMasterViewController.swift
// sapvndemo
//
...
class ACreditMemoReqPartnerMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [ACreditMemoReqPartnerType] = [ACreditMemoReqPartnerType]()
    private let logger = Logger.shared(named: "ACreditMemoReqPartnerTypeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let acreditmemoreqpartnertype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CreditMemoRequest", value: "\(acreditmemoreqpartnertype.creditMemoRequest!)")
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        self.apicreditmemorequestsrvEntities.deleteEntity(currentEntity) { error in
            if let error = error {
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.apicreditmemorequestsrvEntities.fetchACreditMemoReqPartner(matching: query) { aCreditMemoReqPartner, error in
            guard let aCreditMemoReqPartner = aCreditMemoReqPartner else {
                completionHandler(error!)
                return
            }
            self.entities = aCreditMemoReqPartner
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ACreditMemoReqPartnerDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].creditMemoRequest ?? ""
...
extension ACreditMemoReqPartnerMasterViewController: EntitySetUpdaterDelegate {
