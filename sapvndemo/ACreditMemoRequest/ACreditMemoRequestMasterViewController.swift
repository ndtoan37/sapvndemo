//
// ACreditMemoRequestMasterViewController.swift
// sapvndemo
//
...
class ACreditMemoRequestMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [ACreditMemoRequestType] = [ACreditMemoRequestType]()
    private let logger = Logger.shared(named: "ACreditMemoRequestTypeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let acreditmemorequesttype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CreditMemoRequest", value: "\(acreditmemorequesttype.creditMemoRequest!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.apicreditmemorequestsrvEntities.fetchACreditMemoRequest(matching: query) { aCreditMemoRequest, error in
            guard let aCreditMemoRequest = aCreditMemoRequest else {
                completionHandler(error!)
                return
            }
            self.entities = aCreditMemoRequest
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ACreditMemoRequestDetailViewController
...
extension ACreditMemoRequestMasterViewController: EntitySetUpdaterDelegate {
